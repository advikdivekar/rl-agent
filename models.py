...
QuestionField = Literal["age", "income", "occupation", "has_aadhaar"]
DocumentField = Literal["aadhaar_card", "pan_card", "aadhaar", "pan"]
SchemeName = Literal["PMKVY", "MGNREGS", "PMAY"]
...
class Action(BaseAction):
    ...
    value: Optional[str] = Field(
        None,
        description=(
            "Bounded action value. For ask_question use one of: age, income, occupation, has_aadhaar. "
            "For request_document use one of: aadhaar_card, pan_card, aadhaar, pan. "
            "For approve_scheme use one of: PMKVY, MGNREGS, PMAY. "
            "For reject_applicant or escalate use a concise category such as AGE_EXCEEDED, "
            "INCOME_TOO_HIGH, NO_ELIGIBLE_SCHEME, MISSING_REQUIRED_DATA, DATA_MISMATCH, "
            "DOCUMENT_CONFLICT, or MANUAL_REVIEW_REQUIRED."
        )
    )
    ...
    def validate_value(self) -> "Action":
        ...
        if self.action_type == "ask_question":
            allowed: tuple[str, ...] = ("age", "income", "occupation", "has_aadhaar")
