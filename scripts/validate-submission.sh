...
if command -v python >/dev/null 2>&1; then
  PY_COMPILE_OK=true
  PY_COMPILE_OUTPUT=$(cd "$REPO_DIR" && python -m py_compile inference.py models.py server/*.py 2>&1) || PY_COMPILE_OK=false
...
if command -v pytest >/dev/null 2>&1; then
  TEST_OK=true
  TEST_OUTPUT=$(cd "$REPO_DIR" && pytest tests/ -q 2>&1) || TEST_OK=false
