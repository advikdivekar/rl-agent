#!/usr/bin/env bash
#
# validate-submission.sh — Extended OpenEnv Submission Validator
#
...
#   ./scripts/validate-submission.sh https://my-team.hf.space /path/to/repo
#
...
if command -v openenv >/dev/null 2>&1; then
  ...
  OPENENV_VALIDATE_CMD=(docker run --rm "$VALIDATOR_IMAGE_TAG" sh -lc "cd /app/env && python -m openenv_core validate")
else
...
if command -v python >/dev/null 2>&1; then
  ...
  (cd "$REPO_DIR" && python -m py_compile inference.py models.py server/*.py >"$PY_COMPILE_LOG" 2>&1) || PY_COMPILE_OK=false
