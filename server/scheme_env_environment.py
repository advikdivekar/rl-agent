...
# Dense shaping is intentionally shallow so agents cannot reward-farm by asking
# every possible question or requesting unnecessary documents.
...
def generate_dynamic_persona(task_id: int) -> dict:
    """
    Generate a randomized applicant profile for the selected task template.
    Each reset keeps the same task logic but refreshes the underlying applicant
    so an agent must read state rather than memorize a fixed script.
    """
    if task_id == 1:
